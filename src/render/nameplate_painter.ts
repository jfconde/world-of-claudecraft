  ...
  update(fullPass: boolean): void {
    ...
    for (const [id, v] of this.views) {
      ...
      if (e.kind === 'object') {
        ...
        // Level badge goes into its own element so the con color applies only to
        // the bracket, not the mob name. Hidden for dead mobs (corpse has no level).
